...
public final class ImmortalityInvokeImmortality {
    ...
    private static float handlePlayerDeath(ServerPlayerEntity playerEntity, DamageSource dmgSource, float damageAmount) {
            ...
            if (immortalEntity instanceof WolfEntity wolfEntity) {
                if (ImmortalityStatus.getSummonedTeleport(playerEntity) && wolfEntity.isTeammate(playerEntity)) {
                    if (wolfEntity.getWorld() != playerEntity.getWorld() || wolfEntity.distanceTo(playerEntity) > 10) {
                        ...
                        FabricDimensions.teleport(wolfEntity, playerEntity.getWorld(), new TeleportTarget(playerEntity.getPos(), Vec3d.ZERO, wolfEntity.getYaw(), wolfEntity.getPitch()));
                ...
                if (ImmortalityStatus.getSummonedTeleport(playerEntity)) {
                    if (hostileEntity.getWorld() != playerEntity.getWorld() || hostileEntity.distanceTo(playerEntity) > 10) {
                        ...
                        FabricDimensions.teleport(hostileEntity, playerEntity.getWorld(), new TeleportTarget(playerEntity.getPos(), Vec3d.ZERO, hostileEntity.getYaw(), hostileEntity.getPitch()));
        ...
        if ((ImmortalityStatus.getImmortality(playerEntity) || ImmortalityStatus.getLiverImmortality(playerEntity) || ImmortalityStatus.isSemiImmortal(playerEntity) || ImmortalityStatus.isTrueImmortal(playerEntity))
                &&
                ((playerEntity.getHealth() - damageAmount) <= 0 ||
